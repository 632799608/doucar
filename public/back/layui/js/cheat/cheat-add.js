layui.define(['form', 'global', 'upload'], function(exports) {
	...
	$(function() {
		layui.upload({
			url: Aizxin.U('qiniu'),
			elem: '#cheat-upload', //指定原始元素，默认直接查找class="layui-upload-file"
		...
		// 生成编辑器
		var editor = new wangEditor('cheat-content');
		...
		// 秘籍添加
		form.on('submit(addcheatstore)', function(data) {
			if (data.field.articleCategoryId == 0) {
				layer.msg('秘籍分类没有选择', {
					icon: 5,
					shade: 0.5
				});
			...
			axios.post(Aizxin.U('cheat'), data.field)
				.then(function(response) {
					layer.close(index);
					if (response.data.code == 200) {
						layer.msg(response.data.message, {
							...
							window.location.href = Aizxin.U('cheat');
						});
					} else {
						layer.msg(response.data.message, {
							icon: 5,
		...
		// 秘籍修改
		form.on('submit(editcheatupdate)', function(data) {
			if (data.field.articleCategoryId == 0) {
				layer.msg('秘籍分类没有选择', {
					icon: 5,
					shade: 0.5
			...
			axios.put(Aizxin.U('cheat') + "/" + data.field.id, data.field)
				.then(function(response) {
					layer.close(index);
					if (response.data.code == 200) {
						layer.msg(response.data.message, {
