layui.define(['global', 'form', 'laypage'], function(exports) {
	var $ = layui.jquery,
		...
		Aizxin = layui.global;
	$(function() {
		var cheatCategoryList1 = new Vue({
			el: '#cheatCategoryList',
			data: {
				category: [],
				caret: 'caret-right',
				iSndex: -1,
			...
			methods: {
				list: function() {
					var _this = this;
					axios.get(Aizxin.U('cheat/category/create'))
				...
				deleteCategory: function(vo) {
					...
					layer.confirm('确认是否删除', {
						...
						axios.delete(Aizxin.U('cheat/category') + '/' + vo.id).then(function(response) {
								layer.close(index);
		...
		form.on('submit(addArticleCategory)', function(data) {
			...
			axios.post(Aizxin.U('cheat/category'), data.field)
				.then(function(response) {
					layer.close(index);
					if (response.data.code == 200) {
						layer.msg(response.data.message, {
