layui.define(['global', 'form', 'laypage'], function(exports) {
	var $ = layui.jquery,
		...
		Aizxin = layui.global;
	$(function() {
		var waypointCategoryList1 = new Vue({
			el: '#waypointCategoryList',
			data: {
				category: [],
				caret: 'caret-right',
				iSndex: -1,
			...
			methods: {
				...
				deleteCategory: function(vo) {
					...
					layer.confirm('确认是否删除', {
						...
						axios.delete(Aizxin.U('waypoint/category') + '/' + vo.id).then(function(response) {
								layer.close(index);
		...
		// 文章分类添加
		form.on('submit(addWaypointCategory)', function(data) {
			var index = layer.load(1, {
				shade: 0.5
			});
			data.field.status = data.field.status != undefined ? 1 : 0;
			axios.post(Aizxin.U('waypoint/category'), data.field)
				.then(function(response) {
					layer.close(index);
					if (response.data.code == 200) {
						layer.msg(response.data.message, {
							...
							$('#acid').val(0);
							waypointCategoryList1.list();
							waypointCategoryList1.$set('parent_id', 0);
							$('#WaypointCategoryReset')[0].reset();
							form.render('select');
