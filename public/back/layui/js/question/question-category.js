layui.define(['global', 'form', 'laypage'], function(exports) {
	var $ = layui.jquery,
		...
		Aizxin = layui.global;
	$(function() {
		var questionCategoryList1 = new Vue({
			el: '#questionCategoryList',
			data: {
				category: [],
				caret: 'caret-right',
				iSndex: -1,
			...
			methods: {
				...
				deleteCategory: function(vo) {
					...
					layer.confirm('确认是否删除', {
						...
						axios.delete(Aizxin.U('question/category') + '/' + vo.id).then(function(response) {
								layer.close(index);
		...
		// 文章分类添加
		form.on('submit(addQuestionCategory)', function(data) {
			...
			axios.post(Aizxin.U('question/category'), data.field)
				.then(function(response) {
					layer.close(index);
					if (response.data.code == 200) {
						layer.msg(response.data.message, {
							...
							$('#acid').val(0);
							questionCategoryList1.list();
							questionCategoryList1.$set('parent_id', 0);
							$('#QuestionCategoryReset')[0].reset();
							form.render('select');
						});
					} else {
