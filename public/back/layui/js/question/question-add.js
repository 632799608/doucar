layui.define(['form', 'global', 'upload', 'layedit'], function(exports) {
	...
	$(function() {
		...
		// 文章添加
		form.on('submit(addquestionstore)', function(data) {
			data.field.id = 0;
			var url = LAY_question_upload.src;
			data.field.thumb = url.indexOf(window.conf.QINIU_DOMAINS_DEFAULT) > 0 ? url : '';
			data.field.analysis = layedit.getContent(layeditIndex);
			data.field.answer = adAnhtml1.answerList;
			if (adAnhtml1.answerList.length != adAnhtml1.questionType) {
				layer.msg('试题答案为' + adAnhtml1.questionType + '个答案', {
					icon: 2,
					shade: 0.3
				});
				return false;
			...
			axios.post(Aizxin.U('question'), data.field)
				.then(function(response) {
					layer.close(index);
					if (response.data.code == 200) {
						layer.msg(response.data.message, {
		...
		form.on('select(questionType)', function(data) {
			if (data.value == 0) {
				...
				adAnhtml1.$set('questionType', data.value == 1 ? 2 : 4);
			}
		});
		//答案添加
		form.on('submit(answeradd)', function(data) {
			...
			layer.closeAll();
			adAnhtml1.answerList.push(data.field);
			return false;
		});
		// 科目方式
		form.on('select(courseType)', function(data) {
			...
			cateSelect(_con, adAnhtml1.category, $('#questionCategory'))
		});
