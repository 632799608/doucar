...
class PermissionService
{
	...
	public function index()
	{
		...
		try {
			...
			$result->data = $this->permissionRepo->orderBy('id','asc')->paginate($pageSize,['id','name','slug'])->toArray();
		} catch (Exception $e) {
			$result->code = 1001;
	...
	public function store($request)
	{
		...
		try {
	        ...
	        try {
		       ...
		       Cache::forget(config('admin.global.cache.permission'));
			} catch (Exception $e) {
				$result->code = 400;
	...
	public function permissionParentList()
	{
		if(Cache::has(config('admin.global.cache.permission'))){
			return Cache::get(config('admin.global.cache.permission'));
		}
