...
use Exception;
use Aizxin\Validators\CheatCategoryValidator;
use Prettus\Validator\Exceptions\ValidatorException;
use Prettus\Validator\Contracts\ValidatorInterface;
use Cache;
class CheatCategoryService
{
	protected $aCategoryRepo;
	protected $pValidator;
	public function __construct(
		CheatCategoryRepositoryEloquent $aCategoryRepo,
		CheatCategoryValidator $pValidator)
	{
	...
	public function store($request)
	{
		...
		try {
	        ...
	        try {
		       ...
		       Cache::forget(config('admin.global.cache.cheatcategory'));
			} catch (Exception $e) {
				$result->code = 400;
	...
	{
		$result = new Result();
		if(Cache::has(config('admin.global.cache.cheatcategory'))){
			$result->data = Cache::get(config('admin.global.cache.cheatcategory'));
			return $result->toJson();
		}
		try {
			$list = $this->aCategoryRepo->all(['id','parent_id','name','status'])->toArray();
			$data = sort_parent($list);
			Cache::forever(config('admin.global.cache.cheatcategory'),$data);
			$result->data = $data;
        } catch (Exception $e) {
        	$result->code = 422;
	...
	public function destroy($id) {
		$result = new Result();
		try {
			$aCategory = $this->aCategoryRepo->withCount('cheat')->find($id);
			if($aCategory->cheat_count){
