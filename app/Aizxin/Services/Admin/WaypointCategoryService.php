...
use Exception;
use Aizxin\Validators\WaypointCategoryValidator;
use Prettus\Validator\Exceptions\ValidatorException;
use Prettus\Validator\Contracts\ValidatorInterface;
use Cache;
class WaypointCategoryService
{
	protected $aCategoryRepo;
	protected $pValidator;
	public function __construct(
		WaypointCategoryRepositoryEloquent $aCategoryRepo,
		WaypointCategoryValidator $pValidator)
	{
	...
	public function store($request)
	{
		...
		try {
	        ...
	        try {
		       ...
		       Cache::forget(config('admin.global.cache.waypointcategory'));
			} catch (Exception $e) {
				$result->code = 400;
				$result->message = $e;//$id?trans('admin/alert.category.edit_error'):trans('admin/alert.category.create_error');
	...
	{
		$result = new Result();
		if(Cache::has(config('admin.global.cache.waypointcategory'))){
			$result->data = Cache::get(config('admin.global.cache.waypointcategory'));
			return $result->toJson();
		}
		try {
			$list = $this->aCategoryRepo->all(['id','parent_id','name','status'])->toArray();
			$data = sort_parent($list);
			Cache::forever(config('admin.global.cache.waypointcategory'),$data);
			$result->data = $data;
        } catch (Exception $e) {
        	$result->code = 422;
	...
	public function destroy($id) {
		$result = new Result();
		try {
			$aCategory = $this->aCategoryRepo->withCount('point1')->find($id);
			if($aCategory->point1_count){
